...
class TestBetSizeProbability(unittest.TestCase):
    ...
    def test_bet_size_probability_default(self):
        ...
        events_test = pd.DataFrame(
            data=[[0.55, 1], [0.7, -1], [0.95, 1], [0.65, -1], [0.85, 1]], columns=["prob", "side"], index=dates_test
        )
    ...
    def test_bet_size_probability_stepsize(self):
        ...
        self.assertTrue(
            signal_1.equals(
                bet_size_probability(
                    events=events_test, prob=events_test["prob"], num_classes=2, pred=events_test["side"], step_size=0.1
                )
            )
        )
...
class TestBetSizeDynamic(unittest.TestCase):
    ...
    def test_bet_size_dynamic_default(self):
        ...
        events_results["l_p"] = events_results.apply(
            lambda row: limit_price(row.t_pos, row.pos, row.f, w_param, row.max_pos, "sigmoid"), axis=1
        )
        ...
        self.assertTrue(
            df_result.equals(
                bet_size_dynamic(events_test["pos"], events_test["max_pos"], events_test["m_p"], events_test["f"])
            )
        )
...
class TestBetSizeReserve(unittest.TestCase):
    ...
    @patch("mlfinpy.bet_sizing.bet_sizing.most_likely_parameters")
    def test_bet_size_reserve_default(self, mock_likely_parameters):
        ...
        shift_dt = np.array([dt.timedelta(days=d) for d in np.random.uniform(1.0, 20.0, sample_size)])
    ...
    def test_bet_size_reserve_return_params(self, mock_likely_parameters):
        ...
        events_active["bet_size"] = events_active["c_t"].apply(lambda c: single_bet_size_mixed(c, test_fit))
        # Evaluate.
        eval_events, eval_params = bet_size_reserve(
            df_events["t1"], df_events["side"], fit_runs=10, return_parameters=True
        )
...
class TestConfirmAndCastToDf(unittest.TestCase):
    ...
    def test_cast_to_df_one_series(self):
        ...
        events_test = pd.DataFrame(
            data=[
                [25, max_pos, market_price, forecast_price],
    ...
    def test_cast_to_df_no_series(self):
        ...
        events_test = pd.DataFrame(
            data=[[pos, max_pos, market_price, forecast_price]], columns=["pos", "max_pos", "m_p", "f"]
        )
    ...
    def test_get_concurrent_sides_default(self):
        ...
        for idx in events_test.index:
            df_long_active_idx = set(
                events_test[(events_test.index <= idx) & (events_test["t1"] > idx) & (events_test["side"] > 0)].index
            )
