...
from mlfinpy.labeling.labeling import add_vertical_barrier, get_bins, get_events
from mlfinpy.sampling.bootstrapping import get_ind_mat_label_uniqueness, get_ind_matrix
...
class TestSequentiallyBootstrappedBagging(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for prob in [0.5, 0.3, 0.2, 0.1]:
            for window in [2, 5, 10]:
                X["label_prob_{}_sma_{}".format(prob, window)] = (
                    X["label_prob_{}".format(prob)].rolling(window=window).mean()
                )
        ...
        self.price_bars_trim = self.data[
            (self.data.index >= self.X_train.index.min()) & (self.data.index <= self.X_train.index.max())
        ].close
    ...
    def test_sb_bagging_not_tree_base_estimator(self):
        ...
        sb_clf = SequentiallyBootstrappedBaggingClassifier(
            estimator=clf, samples_info_sets=self.samples_info_sets, price_bars=self.price_bars_trim
        )
    ...
    def test_sb_bagging_with_max_features(self):
        ...
        clf = RandomForestClassifier(
            n_estimators=1, criterion="entropy", bootstrap=True, class_weight="balanced_subsample", max_depth=12
        )
    ...
    def test_sb_bagging_float_max_samples_warm_start_true(self):
        ...
        """
        clf = RandomForestClassifier(
            n_estimators=1, criterion="entropy", bootstrap=False, class_weight="balanced_subsample", max_depth=12
        )
    ...
    def test_value_error_raise(self):
        ...
        bagging_clf_1 = SequentiallyBootstrappedBaggingClassifier(
            estimator=clf, samples_info_sets=self.samples_info_sets, price_bars=self.data
        )
        bagging_clf_2 = SequentiallyBootstrappedBaggingClassifier(
            estimator=clf, samples_info_sets=self.samples_info_sets, price_bars=self.data, max_samples=2000000
        )
        bagging_clf_3 = SequentiallyBootstrappedBaggingClassifier(
            estimator=clf, samples_info_sets=self.samples_info_sets, price_bars=self.data, max_features="20"
        )
        bagging_clf_4 = SequentiallyBootstrappedBaggingClassifier(
            estimator=clf, samples_info_sets=self.samples_info_sets, price_bars=self.data, max_features=2000000
        )
        ...
        bagging_clf_6 = SequentiallyBootstrappedBaggingClassifier(
            estimator=clf, samples_info_sets=self.samples_info_sets, price_bars=self.data, warm_start=True
        )
    ...
    def test_sb_classifier(self):
        """
        Test Sequentially Bootstrapped Bagging Classifier. Here we compare oos/oob scores
        to sklearn's bagging oos scores, test oos predictions values
        """
    ...
    def test_sb_regressor(self):
        ...
        sb_reg = SequentiallyBootstrappedBaggingRegressor(
            estimator=reg,
            max_features=1.0,
            n_estimators=100,
            samples_info_sets=self.samples_info_sets,
        ...
        sb_reg_1 = SequentiallyBootstrappedBaggingRegressor(
            estimator=reg,
            max_features=1.0,
            n_estimators=1,
            samples_info_sets=self.samples_info_sets,
