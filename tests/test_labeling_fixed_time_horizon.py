...
class TestLabelingFixedTime(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        test2_actual = pd.DataFrame(
            {
                "SPY": [np.nan, -1, -1, -1, -1, 1, 1, -1, 1, -1],
                "EPP": [np.nan, 1, -1, 1, -1, 1, 1, -1, 1, -1],
                "FXI": [np.nan, -1, -1, 1, -1, 1, 1, -1, 1, -1],
            },
            index=self.idx10,
        )
        test3_actual = pd.DataFrame(
            {
                "SPY": [-1, -1, -1, -1, 1, 1, -1, 1, -1, np.nan],
                "EPP": [1, -1, 1, -1, 1, 1, -1, 1, -1, np.nan],
                "FXI": [-1, -1, 1, -1, 1, 1, -1, 1, -1, np.nan],
            },
            index=self.idx10,
        )
        test4_actual = pd.DataFrame(
            {
                "SPY": [0, -1, 0, -1, 1, 0, 0, 0, -1, np.nan],
                "EPP": [0, -1, 1, -1, 1, 1, -1, 1, -1, np.nan],
                "FXI": [0, -1, 1, -1, 1, 0, -1, 0, -1, np.nan],
            },
            index=self.idx10,
        )
    ...
    def test_dynamic_threshold(self):
        ...
        test6_actual = pd.DataFrame(
            {
                "SPY": [0, -1, -1, -1, 0, 0, 0, 1, 0, np.nan],
                "EPP": [0, -1, 1, -1, 0, 0, 0, 1, 0, np.nan],
                "FXI": [0, -1, 1, -1, 1, 0, 0, 1, 0, np.nan],
            },
            index=self.idx10,
        )
    ...
    def test_with_standardization(self):
        ...
        test8_actual = pd.DataFrame(
            {
                "SPY": [np.nan, np.nan, np.nan, np.nan, 0, 1, 0, 0, 0, -1],
                "EPP": [np.nan, np.nan, np.nan, np.nan, 0, 1, 0, -1, 0, -1],
                "FXI": [np.nan, np.nan, np.nan, np.nan, 0, 1, 0, -1, 0, -1],
            },
            index=self.idx10,
        )
        test9_actual = pd.DataFrame(
            {
                "SPY": [np.nan, np.nan, np.nan, np.nan, 1, 1, -1, 1, -1, np.nan],
                "EPP": [np.nan, np.nan, np.nan, np.nan, 1, 1, -1, 1, -1, np.nan],
                "FXI": [np.nan, np.nan, np.nan, np.nan, 1, -1, -1, 0, -1, np.nan],
            },
            index=self.idx10,
        )
        test10_actual = pd.DataFrame(
            {
                "SPY": [np.nan, np.nan, 1, 0, 1, 0, 0, 1, 0, np.nan],
    ...
    def test_resample(self):
        ...
        test11_actual = pd.DataFrame(
            {
                "SPY": [0, -1, 0, 1, -1, 1, np.nan],
                "EPP": [0, -1, 0, 1, -1, 0, np.nan],
                "FXI": [1, -1, -1, 0, -1, 1, np.nan],
            },
            index=week_index,
        )
        test12_actual = pd.DataFrame(
            {
                "SPY": [np.nan, np.nan, 1, 0, -1, 0, 0, np.nan],
