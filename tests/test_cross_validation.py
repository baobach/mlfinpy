...
class TestCrossValidation(unittest.TestCase):
    ...
    def test_purgedkfold_01_exception(self):
        ...
        try:
            PurgedKFold(n_splits=3, samples_info_sets=samples_info_sets, pct_embargo=0.0)
        except ValueError:
            pass
        else:
    ...
    def test_purgedkfold_02_exception(self):
        """
        Test exception is raised when passing in a dataset with a different length than the ``samples_info_sets``
    ...
    def test_purgedkfold_03_simple(self):
        """
        Test PurgedKFold class using the ``ml_get_train_times`` method. Get the test range from ``PurgedKFold`` and
        then make sure the train range is exactly the same using the two methods.
        ...
        for train_indices, test_indices in pkf.split(dataset):
            self.log(f"test_times_ret=\n{info_sets.iloc[test_indices]}")

            train_times_ret = info_sets.iloc[train_indices]
            self.log(f"train_times_ret=\n{train_times_ret}")

            test_times_gtt = pd.Series(
                index=[info_sets.iloc[test_indices[0]]],
                data=[info_sets.iloc[test_indices[-1]]],
            )
    ...
    def _test_ml_cross_val_score__data(self):
        ...
        records = pd.DataFrame(
            index=info_sets.index,
            data={"even": np.arange(0, sample_size), "odd": np.arange(1, sample_size + 1)},
        )
        ...
        sample_weights = pd.Series(index=info_sets.index, data=random_state.random_sample(sample_size))
    ...
    def test_ml_cross_val_score_02_neg_log_loss(self):
        ...
        self.assertTrue(np.allclose(scores, should_be, rtol=0.1, atol=0.1), "score lists don't match")
    ...
    def test_ml_cross_val_score_03_other_cv_gen(self):
        ...
        self.assertTrue(np.allclose(scores, should_be, rtol=0.05, atol=0.05), "score lists don't match")
