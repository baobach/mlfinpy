...
class M2N:
    ...
    ----------
    moments : list
        The first five (1... 5) raw moments of the mixture distribution.
    epsilon : float, optional
        Fitting tolerance.
    factor : float, optional
        Lambda factor from equations.
    n_runs : int, optional
        Number of times to execute 'singleLoop'.
    ...
    max_iter : int, optional
        Maximum number of iterations to perform in the 'fit' method.
    num_workers : int, optional
        Number of CPU cores to use for multiprocessing execution. Default is -1 which sets num_workers to all cores.
    """
    ...
    def fit(self, mu_2):
        ...
        ----------
        mu_2 : float
            An initial estimate for the mean of the second distribution.
        """
    ...
    def get_moments(self, parameters, return_result=False):
        ...
        ----------
        parameters : list
            List of parameters if the specific order [mu_1, mu_2, sigma_1, sigma_2, p_1].
        return_result : bool, optional
            If True, method returns a result instead of setting the 'self.new_moments' attribute.

        Returns
        -------
        list
            List of the first five moments.
        """
    ...
    def iter_4(self, mu_2, p_1):
        ...
        ----------
        mu_2 : float
            Initial parameter value for mu_2.
        p_1 : float
            Probability defining the mixture; p_1, 1 - p_1.

        Returns
        -------
        list
            List of estimated parameter if no invalid values are encountered (e.g. complex values, divide-by-zero),
    ...
    def single_fit_loop(self, epsilon=0):
        ...
        mu_2 = [
            float(i) * self.epsilon * self.factor * std_dev + self.moments[0] for i in range(1, int(1 / self.epsilon))
        ]
        err_min = self.error
...
def centered_moment(moments, order):
    ...
    -------
    float
        The central moment of specified order.
    """
...
def raw_moment(central_moments, dist_mean):
    ...
    central_moments : list
        The first n (1...n) central moments as a list.
    dist_mean : float
...
def most_likely_parameters(data, ignore_columns="error", res=10_000):
    ...
    ----------
    data : pd.DataFrame
        Contains parameter estimates from all runs.
    ignore_columns : str or list, optional
        Column or columns to exclude from analysis.
    res : int, optional
    ...
    -------
    dict
        Labels and most likely estimates for parameters.
    """
...
def iter_4_jit(mu_2, p_1, m_1, m_2, m_3, m_4):  # pragma: no cover
    ...
    p_1 : float
        Probability defining the mixture; p_1, 1 - p_1.
    m_1, m_2, m_3, m_4 : float
        The first four (1... 4) raw moments of the mixture distribution.
