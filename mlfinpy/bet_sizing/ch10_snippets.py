...
def get_signal(prob, num_classes, pred=None):
    ...
    Calculates the given size of the bet given the side and the probability (i.e. confidence)
    of the prediction. In this representation, the probability will always be
    between 1/num_classes and 1.0.

    Parameters
    ----------
    prob : pd.Series
        The probability of the predicted bet side.
    num_classes : int
        The number of predicted bet sides.
    pred : pd.Series, optional
        The predicted bet side. Default value is None which will return a relative
...
def avg_active_signals(signals, num_threads=1):
    ...
    num_threads : int, optional
        Number of threads to use in multiprocessing, default value is 1.

    Returns
    -------
    pd.Series
        The averaged bet sizes.
    """
...
def mp_avg_active_signals(signals, molecule):
    ...
    ----------
    signals : pd.DataFrame
        Contains at least the following columns: 'signal' (the bet size) and 't1' (the closing time of the bet).
    molecule : list
        Indivisible tasks to be passed to 'mp_pandas_obj', in this case a list of datetimes.

    Returns
    -------
    pd.Series
        The averaged bet size sub-series.
    """
...
def bet_size_sigmoid(w_param, price_div):
    ...
    ----------
    w_param : float
        Coefficient regulating the width of the bet size function.
    price_div : float
...
def inv_price_sigmoid(forecast_price, w_param, m_bet_size):
    ...
    -------
    float
        Inverse of bet size with respect to market price.
    """
...
def get_w_sigmoid(price_div, m_bet_size):
    ...
    -------
    float
        Inverse of bet size with respect to the regulating coefficient.
    """
...
def bet_size_power(w_param, price_div):
    ...
    w_param : float
        Coefficient regulating the width of the bet size function.
    price_div : float
        Price divergence, f - market_price, must be between -1 and 1, inclusive.
    ...
    if not (-1 <= price_div <= 1):
        raise ValueError(
            f"Price divergence must be between -1 and 1, inclusive. Found price divergence value:" f" {price_div}"
        )
    if price_div == 0.0:
        return 0.0
...
def bet_size(w_param, price_div, func):
    ...
    w_param : float
        Coefficient regulating the width of the bet size function.
    price_div : float
        Price divergence, f - market_price
    func : string
...
def inv_price(forecast_price, w_param, m_bet_size, func):
    ...
    float
        Inverse of bet size with respect to market price.
    """
    return {"sigmoid": inv_price_sigmoid, "power": inv_price_power}[func](forecast_price, w_param, m_bet_size)
