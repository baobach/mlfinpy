...
class SequentiallyBootstrappedBaggingClassifier(
    ...
    """
    A Sequentially Bootstrapped Bagging classifier is an ensemble meta-estimator that
    fits base classifiers each on random subsets of the original dataset generated using
    ...
    def _set_oob_score(self, X, y):
        ...
        for estimator, samples, features in zip(
            self.estimators_, self.sequentially_bootstrapped_samples_, self.estimators_features_
        ):
...
class SequentiallyBootstrappedBaggingRegressor(SequentiallyBootstrappedBaseBagging, BaggingRegressor, RegressorMixin):
    """
    A Sequentially Bootstrapped Bagging regressor is an ensemble meta-estimator that
    fits base regressors each on random subsets of the original dataset and then
